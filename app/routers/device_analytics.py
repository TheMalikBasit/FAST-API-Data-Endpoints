...
async def get_available_months(
    ...
    """
    Get available months for analytics (device token auth).
    Returns months based on organization's creation date.
    """
    stmt = select(Organization.created_at).where(Organization.id == current_device.organization_id)
...
async def get_dashboard_stats(
    ...
    """
    Get dashboard statistics for a month (device token auth).
    Returns KPI stats, daily trends, and room-based violation data for the organization.
    """
    ...
    org_filter = (Violation.organization_id == current_device.organization_id)
...
async def get_day_details(
    ...
    """
    Get detailed analytics for a specific day (device token auth).
    Returns hourly breakdown, violation list, and location statistics.
    """
    try:
...
# --- 3. Log Report Download (Called by device after download completes) ---
...
async def log_report_download(
    ...
    """
    Log a report download action (device token auth).
    Called after a device downloads a report. Logs in background.
    """
    filters = {}
    ...
    background_tasks.add_task(
        ...
        user_id=None,
        action="REPORT_DOWNLOADED_BY_DEVICE",
        details={
            "actor_device_name": current_device.name,
            "actor_device_id": str(current_device.id),
